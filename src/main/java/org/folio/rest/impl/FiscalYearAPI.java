          ...
          reply -> {
            try {
              if(reply.succeeded()){
                ...
                List<FiscalYear> fys = (List<FiscalYear>)reply.result().getResults();
                collection.setFiscalYears(fys);
                Integer totalRecords = reply.result().getResultInfo().getTotalRecords();
                collection.setTotalRecords(totalRecords);
                Integer first = 0, last = 0;
                if (fys.size() > 0) {
                  first = offset + 1;
                  last = offset + fys.size();
                }
        ...
        PostgresClient.getInstance(vertxContext.owner(), tenantId).get(FISCAL_YEAR_TABLE, FiscalYear.class, c, true,
          reply -> {
            try {
              if(reply.succeeded()){
                @SuppressWarnings("unchecked")
                List<FiscalYear> userGroup = (List<FiscalYear>) reply.result().getResults();
