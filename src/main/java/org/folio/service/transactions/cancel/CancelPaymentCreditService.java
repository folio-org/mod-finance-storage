...
public class CancelPaymentCreditService implements CancelTransactionService{
  ...
  @Override public Future<Void> cancelTransactions(List<Transaction> tmpTransactions, DBClient client) {
    return getTransactions(tmpTransactions, client)
      .map(transactions -> getVoidedTransactions(tmpTransactions, transactions))
      .compose(transactions -> cancelPaymentCredits(transactions, client))
  ...
  private Future<List<Transaction>> cancelPaymentCredits(List<Transaction> transactions, DBClient client) {
    ...
    return budgetService.getBudgets(getSelectBudgetsQuery(client.getTenantId()), Tuple.of(UUID.fromString(summaryId)), client)
      .map(budgets -> cancelBudgetsTotalsPaymentCredits(transactions, budgets))
      .compose(newBudgets -> budgetService.updateBatchBudgets(newBudgets, client))
      .map(transactions);
  }

  private List<Budget> cancelBudgetsTotalsPaymentCredits(List<Transaction> tempTransactions, List<Budget> budgets) {
    ...
    return tempGrouped.entrySet().stream()
      .map(this::cancelBudgetTotalsPaymentCredits)
      .collect(toList());
  }
  ...
  private Budget cancelBudgetTotalsPaymentCredits(Map.Entry<Budget, List<Transaction>> entry) {
