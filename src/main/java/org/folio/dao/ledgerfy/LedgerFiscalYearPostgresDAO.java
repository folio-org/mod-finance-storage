...
import static org.folio.rest.persist.HelperUtils.handleAsyncResult;
...
public class LedgerFiscalYearPostgresDAO implements LedgerFiscalYearDAO {
  ...
  public Future<Void> deleteLedgerFiscalYearRecords(Criterion criterion, DBClient client) {
    Promise<Void> promise = Promise.promise();
    client.getPgClient().delete(client.getConnection(), LEDGERFY_TABLE, criterion, reply -> HelperUtils.handleAsyncResult(promise, reply));
    return promise.future();
  }
  ...
  public Future<Void> updateLedgerFiscalYears(List<LedgerFY> ledgersFYears, DBClient client) {
    ...
    client.getPgClient().execute(client.getConnection(), sql, reply -> HelperUtils.handleAsyncResult(promise, reply));
    return promise.future();
  }
