  ...
  public Future<List<ExpenseClass>> getExpenseClassesByTemporaryBudgetId(String budgetId, DBClient client) {
    ...
    client.getPgClient()
      .select(getSelectExpenseClassQueryByBudgetId(client.getTenantId()), Tuple.of(budgetId), reply -> {
        if (reply.failed()) {
          logger.error("Getting expense classes by temporary budgetid {} failed", budgetId, reply.cause());
          ...
          logger.info("Successfully retrieved {} expense classes by temporary budgetid {}", budgets.size(), budgetId);
